...
if __name__ == '__main__':
    ...
    save_imgs_to = f'{img_folder}/ppa/ppa_2'
    ...
    dt = 0.5
    net = Network(populations, W, drives, dt, history_len=int(80000 / dt))

    net.run(int(80000 / dt))
    ...
    for i in tqdm(range(80000)):
        if i % 100 == 0:
            ...
            for j in range(100):
                axes[0].plot(v[np.maximum(0, i - j), 0], m[np.maximum(0, i - j), 0], 'bo', alpha=((100 - j) / 100),
                             markersize=0.5)

            axes[1].plot(v[:, 1], m[:, 1], color='orange')
            axes[1].plot(v[i, 1], m[i, 1], 'bo', markersize=5)
            for j in range(100):
                axes[1].plot(v[np.maximum(0, i - j), 1], m[np.maximum(0, i - j), 1], 'bo', alpha=((100 - j) / 100),
                             markersize=0.5)
