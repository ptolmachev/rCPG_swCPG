...
def vectorfield(w, t, p):
    ...
    V = w[:num_nrns]
    M = w[num_nrns:]
    Capacity, gl, gnap, gk, gad, gsynE, gsynI, Ena, Ek, El, EsynE, EsynI, vhalf, kv, tnapmax, tad, kad, b, c, d, t1, t2, amp = p
    ...
    for i in range(1,num_nrns):
        if i != 5:
            ...
            f.append((-Iad[i] - Il[i] - IsynE[i] - IsynI[i] + I_square(t, t1, t2, amp)) / Capacity)
...
def model(b, c, vectorfield, t1, t2, amp, stoptime):
    num_nrns = b.shape[0]
    num_drives = 3
    # Parameter values
    Capacity = 20
