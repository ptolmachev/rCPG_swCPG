...
class Network():
    def __init__(self, populations, synaptic_weights, drives, dt, history_len):
        ...
        self.r_E = 1.0 * np.random.rand(self.N, self.N)
        self.r_I = 1.0 * np.random.rand(self.N, self.N)
        ...
        self.E_ad, self.E_synE, self.E_synI, self.V_half, self.slope, self.K_ad, self.tau_ad, self.tau_NaP_max, self.tau_synE, \
        self.tau_synI = [np.zeros(self.N) for i in range(20)]
        ...
        for i, (name, population) in enumerate(populations.items()):
            ...
            params_list = ["C", "g_NaP", "g_K", "g_ad", "g_l", "g_synE", "g_synI", "E_Na", "E_K", "E_l",
                        "E_ad", "E_synE", "E_synI", "V_half", "slope", "K_ad", "tau_ad", "tau_NaP_max", "tau_synE",
                        "tau_synI"]
    ...
    def r_E_inf(self, v):
        ...
        inputs = self.W_pos * self.firing_rate(v, self.V_half, self.slope).reshape(self.N, 1) + tonic_drives_all
        res = 1.0 / (1.0 + np.exp(-(inputs-1)/ 1))
        return res

    def r_I_inf(self, v):
        inputs = self.W_neg * self.firing_rate(v, self.V_half, self.slope).reshape(self.N, 1)
        res = 1.0 / (1.0 + np.exp(-(inputs-1)/ 1))
        return res
    ...
    def I_SynE(self, v):
        I_synE = self.g_synE * np.sum(self.r_E, axis=0) * (v - self.E_synE)
        return I_synE

    def I_SynI(self, v):
        I_synI = self.g_synI * np.sum(self.r_I, axis=0) * (v - self.E_synI)
        return I_synI
    ...
    def rhs_r_E(self, v, r_E):
        res = (self.r_E_inf(v) - r_E) / self.tau_synE.reshape(self.N, 1)
        return res

    def rhs_r_I(self, v, r_I):
        res = (self.r_E_inf(v) - r_I) / self.tau_synI.reshape(self.N, 1)
        return res
    ...
    def step(self):
        ...
        k_re1 = self.dt * self.rhs_r_E(self.v, self.r_E)
        k_ri1 = self.dt * self.rhs_r_I(self.v, self.r_I)
        ...
        k_re2 = self.dt * self.rhs_r_E(self.v + k_v1 / 2, self.r_E + k_re1 / 2)
        k_ri2 = self.dt * self.rhs_r_I(self.v + k_v1 / 2, self.r_I + k_ri1 / 2)
        ...
        k_re3 = self.dt * self.rhs_r_E(self.v + k_v2 / 2, self.r_E + k_re2 / 2)
        k_ri3 = self.dt * self.rhs_r_I(self.v + k_v2 / 2, self.r_I + k_ri2 / 2)
        ...
        k_re4 = self.dt * self.rhs_r_E(self.v + k_v3, self.r_E + k_re3)
        k_ri4 = self.dt * self.rhs_r_I(self.v + k_v3, self.r_E + k_ri3)
        ...
        new_r_E = self.r_E + 1.0 / 6.0 * (k_re1 + 2 * k_re2 + 2 * k_re3 + k_re4)
        new_r_I = self.r_I + 1.0 / 6.0 * (k_ri1 + 2 * k_ri2 + 2 * k_ri3 + k_ri4)
...
if __name__ == '__main__':
    ...
    file = open("../../data/params/rCPG_swCPG.json", "rb+")
